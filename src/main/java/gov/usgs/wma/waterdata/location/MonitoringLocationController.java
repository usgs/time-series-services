...
public class MonitoringLocationController {
	...
	@Operation(
			...
			responses= {
					@ApiResponse(
							responseCode="200",
							description="GeoJSON repreesentation of the Monitoring Location.",
							content=@Content(schema=@Schema(implementation=GeoJSON.class))),
					@ApiResponse(
							responseCode="404",
	...
	@GetMapping(value="monitoring-location/{monitoringLocationId}")
	public GeoJSON getMonitoringLocation(@PathVariable(value="monitoringLocationId") String monitoringLocationId,
			HttpServletResponse response) {
		GeoJSON rtn = monitoringLocationDao.getLocation(monitoringLocationId);
		if (null == rtn) {
